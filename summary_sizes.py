...
def main(dataset_dirs, output_dir, from_csv=False):
    ...
    print(f"Saved scatter plot to {out_path}")
        ...
        # ✅ Pixel count (area) histogram overlay
    plt.figure(figsize=(10, 6))
    colors = plt.cm.tab10(np.linspace(0, 1, len(datasets)))

    all_areas = np.concatenate([a for _, _, a in datasets.values()])
    area_max = np.percentile(all_areas, 99)  # clip outliers for visualization

    for i, ((name, (_, _, areas)), color) in enumerate(zip(datasets.items(), colors)):
        clipped = areas[areas <= area_max]
        plt.hist(
            clipped,
            bins=100,
            alpha=0.6,
    ...
    #plt.title("Distribution of Image Pixel Counts Across Datasets", fontsize=18)
    plt.legend(fontsize=14,loc="best")
    plt.grid(True, which="both", alpha=0.3)
    plt.tight_layout()

    pixel_hist_path = os.path.join(output_dir, "pixel_count_histogram.png")
    plt.savefig(pixel_hist_path, dpi=300)
    plt.close()
    print(f"Saved pixel count histogram to {pixel_hist_path}")
